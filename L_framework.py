...
##### try to see if fomration can be achieved by local coordinate framework ####
...
class Agent:
    def __init__(self,ID,p,B,stats,L,D,T,is_leader):
        ...
        self.p = p  
        #######################
        self.theta = 2*3.14*np.random.rand()    # coordinate rotation matrix
        self.p = self.trans_frame(self.p, 'g2l')
        #######################
    ...
    def trans_frame(self,pos,mode):
        ...
        if mode=='g2l':
            ...
            out = np.dot(pos,inv(Rot))
        
        return out
    
    def current_pos(self):
        return self.trans_frame(self.z, 'l2g')
